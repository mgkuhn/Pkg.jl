...
@testset "Artifact Usage" begin
    ...
    for flooblecrank_status in ("engaged", "disengaged")
        temp_pkg_dir() do project_path
            ENV["FLOOBLECRANK"] = flooblecrank_status
            ...
            artifacts_toml = joinpath(project_path, "AugmentedPlatformHook", "Artifacts.toml")
            ...
            Pkg.activate(joinpath(project_path, "AugmentedPlatformHook"))
            add_this_pkg()
            Pkg.instantiate(; verbose=true)
            ...
            run(`$(Base.julia_cmd()) --project=$(Base.active_project()) -e 'using Pkg; Pkg.test("AugmentedPlatformHook")'`)
        end
    end
end
